...
- **Bug en preview de Vercel — Deployment Protection:** el primer deploy preview falló el PDF con `TimeoutError: Waiting for function failed` (`__deckPrintReady` nunca true). Causa: el preview tiene **Vercel Authentication** activa → la función, al pedir su PROPIA URL server-to-server (sin la cookie de sesión del usuario), recibe el muro de SSO (`HTTP 401 Authentication Required`, `vercel.com/sso`) en vez del deck. Chromium sí arrancó (el `engines.node:22.x` funcionó). Fix elegido (mantener el preview privado): **Protection Bypass for Automation** → Vercel expone `VERCEL_AUTOMATION_BYPASS_SECRET`; `api/pdf.js` ahora manda ese secreto como header `x-vercel-protection-bypass` en cada request (vía `page.setExtraHTTPHeaders`, cubre documento + css/js/assets). Si la env var no está, el header no se manda (inerte). Requiere activar el bypass en Settings → Deployment Protection y redeploy.
Próximo paso: con el bypass activado, validar el PDF en el preview (incl. `&hidden=…`).
